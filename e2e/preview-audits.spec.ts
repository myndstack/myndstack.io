...
import { engineHull, intersects, textRects, toHold, type Rect } from "./helpers";

/**
 * Whole-page audits of the landing's composition (poster mode):
 *
 * - stacking: sections make no stacking context and never overlap, and every
 *   word paints above the engine;
 * - overlap: at every hold, at every pinned size, no text comes within 8px of
 *   the engine — and in the static layout, of any block's own poster;
...
async function open(page: Page, size: { width: number; height: number }) {
  ...
  await expect(page.locator(`[data-chapter="core-hero"]`)).toHaveAttribute("data-motion", "done", { timeout: 8000 });
  // Settle every once-chapter so nothing is mid-rise while we measure.
  await page.addStyleTag({ content: "*,*::before,*::after{transition:none!important;animation:none!important}" });
}

test.describe("stacking contract", () => {
  test("sections make no stacking context and never overlap", async ({ page }) => {
    await open(page, { width: 1440, height: 900 });
    const report = await page.evaluate(() => {
      const blocks = [
        document.querySelector(".landing")!,
        document.querySelector(".engine-run")!,
        document.querySelector(".engine-flow")!,
        ...Array.from(document.querySelectorAll(".engine-flow > *, .landing > section, .landing > .engine-run ~ section")),
      ...
      const sections = Array.from(document.querySelectorAll<HTMLElement>(".landing [data-surface]"))
        .filter((el) => !el.parentElement?.closest("[data-surface]"))
        .map((el) => {
          const r = el.getBoundingClientRect();
          return { id: el.id || el.className, top: r.top + scrollY, bottom: r.bottom + scrollY };
        })
        .sort((a, b) => a.top - b.top);
      const overlaps = sections.slice(1).flatMap((s, i) => (s.top < sections[i].bottom - 1 ? [`${sections[i].id} ↔ ${s.id}`] : []));
      return { checked: blocks.length, sections: sections.length, contexts, overlaps };
    });
    expect(report.checked).toBeGreaterThan(8);
    expect(report.sections).toBeGreaterThan(8);
    expect(report.contexts).toEqual([]);
    expect(report.overlaps).toEqual([]);
  });

  test("every word paints above the engine", async ({ page }) => {
    await open(page, { width: 1440, height: 900 });
    ...
    let checked = 0;
    let underStage = 0;
    for (const id of ["hero", "stack", "cap-1", "work", "build-2", "tools", "studio"]) {
      ...
      const result = await page.evaluate(() => {
        ...
        stage.inert = false;
        const flow = Array.from(document.querySelectorAll<HTMLElement>(".engine-flow :is(h1, h2, h3, p, li, a, button, dt, dd)"));
        const buried: string[] = [];
        let n = 0;
        let under = 0;
        for (const el of flow) {
          const r = el.getBoundingClientRect();
          if (r.width < 4 || r.height < 4 || r.bottom < 0 || r.top > innerHeight || !el.checkVisibility()) continue;
  ...
  for (const size of PINNED) {
    test(`every hold at ${size.width}×${size.height}`, async ({ page }) => {
      ...
      for (const beat of STAGE_BEATS) {
        ...
        expect(hull, `${beat.id}: no engine on screen`).not.toBeNull();
        const text = await textRects(page);
        lines += text.length;
        for (const t of text) {
          if (intersects(t, hull as Rect, PAD)) hits.push(`${beat.id}: "${t.text}"`);
        }
      }
  ...
  for (const size of [
    ...
    test(`static layout at ${size.width}×${size.height}: no block's poster under its words`, async ({ page }) => {
      ...
      const report = await page.evaluate((pad) => {
        ...
        // A poster's drawn bounds, clipped to its own section: the sections
        // after it paint over anything that hangs below (the hero's horizon).
        const posters = Array.from(document.querySelectorAll<HTMLElement>(".engine-slot, .engine-dock"))
          .filter((el) => el.getClientRects().length)
          .map((el) => {
            const drawn = (el.querySelector("use, .engine-bezel circle:nth-child(2)") ?? el).getBoundingClientRect();
            const own = (el.closest("[data-surface]") ?? el).getBoundingClientRect();
        ...
        for (let node = walker.nextNode(); node; node = walker.nextNode()) {
          ...
          if (!node.textContent?.trim() || !parent || parent.closest(".engine-slot, .engine-dock, .sr-only, .mobile-cta, script, style")) continue;
      ...
      for (let y = 0; y <= height - vh; y += vh / 10) {
        ...
        const shown = await page.evaluate(() => {
          ...
          if (stage && getComputedStyle(stage).display !== "none") {
            const on = stage.querySelector("[data-on]");
            if (on && inBand(on.getBoundingClientRect())) return true;
          }
          for (const el of document.querySelectorAll(".engine-slot .ep, .engine-dock .ep")) {
          ...
          for (let node = walker.nextNode(); node; node = walker.nextNode()) {
            ...
            if (node.parentElement.closest("script, style, .sr-only, .ruler")) continue;
            range.selectNodeContents(node);

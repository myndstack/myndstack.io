...
export default function Manifesto({ lead, keep }: { lead: string; keep: string }) {
  ...
  const [ref, inView] = useInView<HTMLHeadingElement>(0.55);
  ...
  return (
    <section className="mx-auto max-w-[1120px] px-5 py-section sm:px-14">
      <div className="eyebrow mb-[26px]">Manifesto</div>
      <h2 ref={ref} className="h1-statement">
        {WORDS.map((word, i) => (
          <span
            key={`${word}-${i}`}

...
export default function StackStory() {
  ...
  return (
    ...
    >
      {/* Scroll anchor for the "Product" nav item — deliberately not on the
          section itself. Hash-navigating to `#platform` lands the browser flush
          at the section top, which is `progress = 0` in the reveal formula
          below → every tile at `opacity: 0` until the visitor scrolls. The
          Product click looked like it dropped you on an empty section.

          This span sits 120vh into the section on desktop, so anchor nav lands
          at scrollY = sectionTop + 120vh → `progress ≈ 0.5`: layers 0 and 1
          locked, 2 nearly locked, 3 partially cascading. Reads as "story in
          progress" the moment you arrive; forward scroll still finishes the
          assembly. On mobile the layer cascade doesn't render at all
          (right column is `hidden sm:block`), so the anchor collapses to the
          section top there — same as before, no regression.

          Not the section's own id: the section keeps `#platform` for the
          scroll-spy (`SPY_IDS`), the a11y focus ring (`tabIndex={-1}`), and
          any deep-links that already point at it. The nav's Product link
          points at THIS anchor instead. */}
      <span
        id="platform-anchor"
        aria-hidden="true"
        className="pointer-events-none absolute left-0 top-0 sm:top-[120vh]"
      />
      ...
      >
        ...
        <div className="relative z-1 mx-auto grid w-full max-w-[1200px] grid-cols-1 items-center gap-16 px-5 sm:px-14 md:grid-cols-[1.05fr_1fr]">
          <div>
            ...
            <p className="m-0 max-w-[420px] text-17 leading-body text-t4">
          ...
          <div className="relative hidden h-[452px] sm:block">
            {STACK_LAYERS.map((layer, i) => (
              ...
              >
                ...
                <span className="ease-brand size-2 flex-none bg-line-3 transition-[background-color,box-shadow] duration-(--dur-base) [.is-locked_&]:bg-lime [.is-locked_&]:shadow-glow" />
              </div>
            ))}
          </div>
        </div>

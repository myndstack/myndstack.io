/**
 * §06 — plays once on entry: each group's hue bar grows and the marks rise
 * into place.
 */
...
export default function buildTools({ root }: ChapterContext): ChapterMotion {
  ...
  tl.add(all(".rack-bar"), { scaleX: [0, 1], duration: 600, delay: stagger(STAGGER.block) }, 0);
  tl.add(all(".rack-item"), { opacity: [0, 1], y: [14, 0], duration: 500, delay: stagger(STAGGER.char) }, 160);
  return { timeline: tl };
}

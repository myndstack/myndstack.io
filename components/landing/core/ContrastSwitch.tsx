 ...
 * set as a split diff (the agency's lines removed, ours added). Both columns
 * are always in the DOM and readable (no JS: a plain comparison table); the
 * switch only moves the emphasis — and the ring beside it, which breaks into
 * gapped, desaturated arcs (the hand-offs) or closes into the spectrum. It
 * also tells the engine (the stage shows the same engine, built the way an
...
export default function ContrastSwitch({ without, with: withUs }: Props) {
  ...
  return (
    <div className="contrast" data-state={ours ? "ours" : "agency"}>
      ...
      <p className="diff-file" aria-hidden="true">
        <span>studio.diff</span>
        <span>
          <b className="diff-del">−{without.length}</b> <b className="diff-add">+{withUs.length}</b>
        </span>
      </p>
      <table className="contrast-table">
        ...
        <tbody>
          {Array.from({ length: rows }, (_, i) => (
            <tr key={i}>
              <td data-side="agency">
                <span className="diff-ln" aria-hidden="true">
                  {i + 1}
                </span>
                {without[i] ?? ""}
              </td>
